...
# --- NEW: Import the shared components ---
...
def ingest_one_clip(mp3_path, tonic_model, ftanet_model, mule_embedder):
    ...
    try:
        ...
        # --- 2.3 Run expensive, non-augmented feature extraction ONCE ---
        emb_vec = mule_embedder.predict(y_base, sr)
        if emb_vec is None:
            # If MULE embedding fails, we can't proceed with this clip
            return []
        ...
        for shift in AUG_TONIC_SHIFTS:
            # Augment the pitch contour directly, much faster!
            # A semitone shift is equivalent to adding/subtracting 100 cents.
    ...
    # Explicitly trigger garbage collection at the end of the function
    # to release memory from large audio arrays (y, y_base, etc.)
    gc.collect()
    return points


if __name__ == "__main__":
    # Increased timeout to 60s to give Qdrant more time to ingest large batches
